...
interface InfiniteMenuLayoutProps {
  photos: Photo[];
  onPhotoClick?: (photo: Photo) => void;
}

export const InfiniteMenuLayout: FC<InfiniteMenuLayoutProps> = ({ 
  photos, 
  onPhotoClick 
}) => {
  ...
  const menuItems = photos.map(photo => ({
    image: photo.url,
    link: photo.id,
    title: photo.title || 'Photo',
    description: photo.backstory || photo.location || 'Gallery photo'
  }));
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      const itemIndex = index % photos.length;
      setActivePhoto(photos[itemIndex]);
    };

    if (canvas && photos.length > 0) {
      sketch = new InfiniteGridMenu(
        canvas,
        menuItems.length ? menuItems : defaultItems,
        handleActiveItem,
        setIsMoving,
        (sk) => sk.run()
      );
    ...
    return () => {
      ...
      if (sketch) {
        // No explicit destroy method, but could add if needed
      }
    };
  }, [photos]);

  const handlePhotoClick = () => {
    if (activePhoto && onPhotoClick) {
